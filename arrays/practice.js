...
// function maker(num1, num2){
//   let makerArray = [];
//   for(let num = num1; num <=num2+1; num++){
//     makerArray.push(num)
//   }
//   return makerArray
// }
...
function addTen(numbers){
  ...
  for(let i = 0; i < numbers.length; i++){
    tenAdded.push(Number(numbers[i]));
    tenAdded[i]+=10;
  }
  return tenAdded
}
...
//Code Here
  ...
  function absenceLeave(arr, name){
    for(let i = 0; i < arr.length; i++){
      if (arr[i] === name){
        arr.splice(i,1)
      }
...
absenceLeave(devMountainEmployees, cahlan)
...
*/
  ...
  function userMaker(arr, name, email, password, username ){
    let user = {
      name, 
      email,
      password,
...
// function deleteUser(arr, key, val){
...
// deleteUser(users, 'mark.mciver@devmountai.in')
